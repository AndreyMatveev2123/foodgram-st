...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Create Users
        users_to_create = [
            {'email': 'testuser1@example.com', 'username': 'testuser1', 'first_name': 'Тест', 'last_name': 'Пользователь1', 'password': 'testpass123'},
            {'email': 'testuser2@example.com', 'username': 'testuser2', 'first_name': 'Тест', 'last_name': 'Пользователь2', 'password': 'testpass123'},
        ]
        ...
        if not created_users:
            self.stdout.write(self.style.ERROR('No users to create recipes for. Please create users first.'))
            return
        
        ingredients = list(Ingredient.objects.all()[:5]) # Get some existing ingredients
        tags = list(Tag.objects.all()[:3]) # Get some existing tags

        if not ingredients:
            self.stdout.write(self.style.ERROR('No ingredients found. Please import ingredients first.'))
            return
        if not tags:
        ...
        for i, user in enumerate(created_users):
            ...
            recipe_text = f'Это тестовый рецепт, созданный пользователем {user.username}.'
